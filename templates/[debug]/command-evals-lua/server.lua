local function evaluateLua(code)
  local func, err = load(code, "User Code: " .. code)
...
RegisterCommand("evals-lua", function(source, _args, rawCommand)
  local code = rawCommand:sub(10)

  if code == "" then
    TriggerClientEvent("chat:addMessage", source, { args = {"Eval", "No code provided."} })
    return
  end
